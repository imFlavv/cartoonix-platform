...
backend:
  ...
  - task: "Admin import episodes from real VPS folder (POST /api/admin/import-folder)"
    implemented: true
    working: true
    file: "backend/server.py"
    stuck_count: 0
    ...
    status_history:
        -working: "NA"
        -agent: "main"
        -comment: "New admin-only endpoint. Body {folder}. Accepts absolute path under VIDEO_DIR or a path relative to VIDEO_DIR. Rejects folders outside VIDEO_DIR (400) and non-existent folders (404). Scans non-recursively, filters video extensions (.mp4,.mkv,.webm,.mov,.m4v,.avi,.wmv,.flv,.mpeg,.mpg,.ts), sorts naturally, parses episode number from SxxEyy (fallback first number then index), prettifies title (strips extension + trailing [id] bracket token), builds video_url=/media/videos/<rel>. Returns {count, episodes:[{number,title,video_url,duration}], folder}. NON-destructive (does not touch DB). Non-admin -> 403. Verified manually via curl on /media/videos/ATOM: 2 episodes, S01E01->1, S01E02->2, titles cleaned."
        -working: true
        -agent: "testing"
        -comment: "✅ ALL TESTS PASSED (6/6). A1: Admin POST with folder='ATOM' returns 200 with count=2, folder='/media/videos/ATOM', episodes array with 2 items. Episode 1: number=1, title='Alpha Teens On Machines A.T.O.M S01E01 - Evadarea lui Paine' (correctly stripped .mp4 extension and [WmJx1] bracket), video_url starts with '/media/videos/ATOM/'. Episode 2: number=2, title='Alpha Teens On Machines A.T.O.M S01E02 - Atingerea' (correctly stripped [aBc2]). A2: Admin POST with absolute path '/media/videos/ATOM' returns 200 with same 2 episodes. A3: Admin POST with folder='/etc' returns 400 (outside VIDEO_DIR protection working). A3b: Admin POST with folder='../../etc' returns 400 (path traversal protection working). A4: Admin POST with non-existent folder 'NOPE_DOES_NOT_EXIST' returns 404. A5: Admin POST with empty folder '' returns 400. A6: Test user (non-admin) POST with folder='ATOM' returns 403 (admin-only protection working). Episode parsing, title cleaning, and security validations all working correctly."
