...
backend:
  - task: "WatchParty full flow (create/invite/respond/playlist/control/leave/end + FREE vs PLUS limits)"
    implemented: true
    working: true
    file: "backend/server.py"
    stuck_count: 0
    ...
    status_history:
        -working: "NA"
        -agent: "main"
        -comment: "New WatchParty feature (collection watchparties, polling-based sync). Endpoints (all auth): POST /watchparty/create {playlist?:[{show_id,episode_number}]} (ends prev active room of owner, max_others=4 if plus else 1); GET /watchparty/current; GET /watchparty/invitations; GET /watchparty/{id} (participant or pending-invitee only, else 403); POST /watchparty/{id}/invite {username} owner-only, finds user by exact nickname (case-insensitive), enforces capacity (accepted others + pending < max_others) else 400, creates notification type=watchparty_invite with room_id; POST /watchparty/{id}/respond {accept} invited-only -> accept adds to participants (409/400 if full), decline marks declined; POST /watchparty/{id}/playlist {action:add/remove/reorder,...} owner-only; POST /watchparty/{id}/control {action:play/pause/seek/next/prev/select,position?,index?} owner-only (non-owner 403); POST /watchparty/{id}/leave (owner leaving ends room); POST /watchparty/{id}/end owner-only. Manually verified happy path via curl: admin(plus) create max_others=4, invite 'Cont Test' pending + notification, test user sees invitation + notification, accept -> participants[Admin,Cont Test], owner control play ok, non-owner control 403, end ok. NEEDS: verify FREE owner max_others=1 (2nd invite -> 400), PLUS owner can invite up to 4, duplicate invite -> 400, invalid username -> 404, self-invite -> 400, non-participant GET room -> 403, respond without invite -> 404."
        -working: true
        -agent: "testing"
        -comment: "✅ ALL WATCHPARTY TESTS PASSED (12/12). Comprehensive testing of all WatchParty endpoints and edge cases. Test 1: PLUS user (admin) creates room with max_others=4, is_owner=true, 1 participant (Admin), 1 playlist item, status=active. Test 2: GET /api/watchparty/current returns correct room. Test 3: Invite by username 'Cont Test' works - invited list contains user with status=pending, slots_used=1, notification created (type=watchparty_invite, room_id matches), invitation appears in GET /api/watchparty/invitations. Test 4: Invalid username 'no_such_user_xyz' correctly returns 404. Test 5: Self-invite (admin inviting 'Admin') correctly returns 400. Test 6: Accept invitation works - participants now include both Admin and Cont Test (2 participants). Test 7: Duplicate invite (inviting 'Cont Test' again) correctly returns 400. Test 8: Control permissions work - owner (admin) can play (is_playing=true) and next actions (200), non-owner (test user) pause blocked with 403. Test 9: Access control after leaving - test user leaves (ended=false for non-owner), then GET room returns 403 (no longer participant). Test 10: FREE user capacity - test user creates room with max_others=1, first invite to Admin works (slots_used=1), duplicate invite correctly returns 400 (capacity enforcement working). Test 11: End room as owner - ended=true, GET /api/watchparty/current returns null. Test 12: Respond without invitation (to ended room) correctly returns 404. All WatchParty features working perfectly: room creation, PLUS vs FREE limits (4 vs 1), invite by username, notifications, invitations list, accept/decline, duplicate prevention, self-invite prevention, control permissions, access control, leave, end, and all error cases."
...
agent_communication:
    -agent: "testing"
    -message: "✅ WATCHPARTY BACKEND TESTING COMPLETE - ALL TESTS PASSED (12/12). Comprehensive testing of the NEW WatchParty feature confirms all endpoints and edge cases are working perfectly. Test credentials: admin@cartoonix.ro/admin1234 (PLUS user, nickname 'Admin'), test@cartoonix.ro/test1234 (FREE user, nickname 'Cont Test'). Used show 'Captain Nova' (id=6a79f499222ca6b10b391fc8) episode 1 for testing. All 12 test cases passed: (1) CREATE as PLUS user - room created with max_others=4, is_owner=true, 1 participant (Admin), 1 playlist item, status=active. (2) GET /api/watchparty/current returns correct room. (3) INVITE by username 'Cont Test' - invited list contains user with status=pending, slots_used=1, notification created (type=watchparty_invite), invitation appears in GET /api/watchparty/invitations. (4) Invalid username 'no_such_user_xyz' correctly returns 404. (5) Self-invite (admin inviting 'Admin') correctly returns 400. (6) Accept invitation - participants now include both Admin and Cont Test (2 participants). (7) Duplicate invite correctly returns 400. (8) Control permissions - owner can play/next (200, is_playing=true), non-owner pause blocked (403). (9) Access control - test user leaves (ended=false), then GET room returns 403. (10) FREE user capacity - test user creates room with max_others=1, first invite works (slots_used=1), duplicate invite returns 400 (capacity enforcement working). (11) End room - ended=true, GET /api/watchparty/current returns null. (12) Respond without invitation correctly returns 404. All WatchParty features working: room creation, PLUS vs FREE limits (4 vs 1), invite by username, notifications, invitations list, accept/decline, duplicate prevention, self-invite prevention, control permissions, access control, leave, end, and all error cases. Cleanup complete - all test rooms ended."

backend:
  - task: "Login endpoint robustness (PHP bcrypt compatibility + missing password_hash handling)"
    implemented: true
    working: true
    file: "backend/server.py"
    stuck_count: 0
    ...
    status_history:
        -working: "NA"
        -agent: "main"
        -comment: "Fixed login endpoint to handle edge cases: (1) PHP-style bcrypt hashes ($2y$/$2x$ variants) are normalized to $2b$ for python-bcrypt compatibility via verify_password helper. (2) Missing password_hash field now handled gracefully using get_stored_hash(user) instead of direct user['password_hash'] access (which threw KeyError). Changed line 493 in server.py from user['password_hash'] to get_stored_hash(user). Test users created: legacy2y@test.ro (has $2y$ hash for password 'parola123'), nohash@test.ro (no password_hash field)."
        -working: true
        -agent: "testing"
        -comment: "✅ ALL TESTS PASSED (8/8). Comprehensive testing of login endpoint robustness confirms all edge cases are handled correctly. Test 1a: Normal login admin@cartoonix.ro/admin1234 returns 200 with token and user object (role=admin). Test 1b: Normal login test@cartoonix.ro/test1234 returns 200 with token. Test 2a: PHP bcrypt ($2y$) with correct password (legacy2y@test.ro/parola123) returns 200 - backend correctly normalizes $2y$ to $2b$. Test 2b: PHP bcrypt ($2y$) with wrong password returns 401 (NOT 500) - clean error handling. Test 3: Missing password_hash field (nohash@test.ro) returns 401 (NOT 500) with clean error message 'Email sau parolă incorectă' - no server crash. Test 4: Wrong password for admin returns 401. Test 5: Non-existent email (nobody@nowhere.ro) returns 401. Backend health check: GET /api/shows returns 200. BUG FIX VERIFIED: The KeyError crash when password_hash field is missing has been fixed by using get_stored_hash(user) instead of direct field access. All login scenarios working correctly with proper error handling."
